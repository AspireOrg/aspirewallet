...
# runs a local nginx server that proxies requests to a remote aspirewallet instance
...
export DEV_REMOTE_HOST=${DEV_REMOTE_HOST:=wallet.aspirewallet.io}
...
envsubst "$VARS" < ./etc/includes/aspirewallet.conf.template > ./etc/includes/aspirewallet.conf
