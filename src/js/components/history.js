function BalanceHistoryViewModel() {
  ...
  self.availableAssets = !USE_TESTNET ? ko.observableArray(['ASP', 'GASP']) : ko.observableArray(['ASP']);
  //^ don't load GASP as an asset on testnet, since we can't show the data (since blockchain doesn't support testnet)
  self.graphData = null;
  ...
  self.init = function() {
    ...
    failoverAPI("get_owned_assets", {addresses: WALLET.getAddressesList()}, function(data, endpoint) {
      ...
      //^ this way, ASP and GASP stay at the top, but the other assets are alphabetically sorted
    });
  ...
  self.assetChanged = function() {
    ...
    if (self.selectedAsset() == 'GASP') { //mainnet only (as we use blockchain.info for this and they don't support testnet)
      var addresses = WALLET.getAddressesList();
      self.graphData = [];

      for (var i = 0; i < addresses.length; i++) {
        //since we don't track GASP balances, we need to go to blockchain.info for that
        ...
        $.queryYQL(q, function(data, textStatus, jqXHR) {
          ...
          var address = /address%3D([A-Za-z0-9]+)%22/g.exec(jqXHR.url)[1];
          var addressHash = hashToB64(address);
          $.jqlog.debug("Could not get GASP balance from blockchain for address " + address + ": " + errorThrown);
          var addressName = PREFERENCES['address_aliases'][addressHash] ? "<b>" + PREFERENCES['address_aliases'][addressHash] + "</b> (" + address + ")" : address;
